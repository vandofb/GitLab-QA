describe Gitlab::QA::Framework::Page::Base do
  ...
  describe '.view', 'DSL for defining view partials' do
    ...
    it 'makes it possible to define page views' do
      ...
      expect(subject.views).to all(be_an_instance_of(Gitlab::QA::Framework::Page::View))
    end

    it 'populates views objects with data about elements' do
      expect(subject.elements.size).to eq 3
      expect(subject.elements).to all(be_an_instance_of(Gitlab::QA::Framework::Page::Element))
