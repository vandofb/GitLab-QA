describe Gitlab::QA::Framework::Factory::Dependency do
  ...
  describe '#build!' do
    ...
    context 'when dependency has not been overridden' do
      ...
      context 'when receives a caller factory as block argument' do
        let(:dependency) { Gitlab::QA::Framework::Factory::Base }

        it 'calls given block with dependency factory and caller factory' do
          allow_any_instance_of(Gitlab::QA::Framework::Factory::Base).to receive(:fabricate!).and_return(factory)
          allow(Gitlab::QA::Framework::Factory::Product).to receive(:populate!).and_return(spy('any'))

          subject.build!

          expect(block).to have_received(:call).with(an_instance_of(Gitlab::QA::Framework::Factory::Base), factory)
        end
      end
    end
  end
